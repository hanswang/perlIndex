...
sub main
{
    my %clist = ();
    for my $f (1 .. 138) {
        my $file = sprintf '<store/pg-%03d.html', $f;

        warn "processing file: $file ...\t";
        local $/;
        my $info;
        unless(open $info, $file){
            warn "Can't oepn data";
            next;
        }
        my $html = <$info>;
        ...
        if ($count < $itcount) {
            ...
            my $newSize = $dom->find('div#international .accordion .content:first-of-type .course-container')->size;
            for(my $i = 1; $i < $itcount - $count + 1 && $i <= $newSize; $i++) {
                # coursename
